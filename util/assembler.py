...
def assemble():
    ...
    # here we get the absolute time of the 18th note and store the time value to "t"
    t = analyzer.get_time(query=17, by="note_id")

    # then we add a text object "elegant" and hatch it at time t with color "#F00"
    # hatch is similar with "initialize", as a result, it creates a text with 0 opacity
    elegant_text = Text("elegant").hatch(at=t, init={"color": "#F00"})
